...
const defaultQaDatabase = [
  {
    ...
    answerZh: "您可以通过打开 应用程序并提供所需信息来再次注册。但是，如果您取消会员资格或取消应用程序，某些数据可能会丢失。此外，如果您多次人同一设备取消订阅并重新注册，您可能无法进行新的注册。在这种情况下，请使用不同的电子邮件地址。感谢您的理解与合作。",
    answerJa: "アプリを開き、必要な情報を提供することで再登録することができます。ただし、同じデバイスで複数回退会と再登録を繰り返すと、お客様のご利用状況によっては新規登録ができないことがあります。キャラクターを作成する際は別のメールアドレスをお試しください。ご理解とご協力をお願いいたします。"
  },
  {
    id: 16,
    ...
    answerZh: "一些用户正在重新创建账户以重置其角色，但使用相同信息创建账户的次数是有限制的。 如果您收到无法创建的消息，请尝试使用其他电子邮件地址（如果有）。我们目前正在考虑在未来的更新中实现对重置功能，因此我们正式不建议删除并重新创建的账户。在使用之前通知了解这一点。",
